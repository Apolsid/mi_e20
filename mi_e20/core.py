...
class Pack(object):
	...
	def _createContent(self):
		...
		for i in range(len(self._list)):
			item_data = self._list[i].getFileData()
			item_len = len(item_data)

			table_head[i*8 : i*8 + 8] = offset.to_bytes(4, byteorder='little') + item_len.to_bytes(4, byteorder='little')
			table_body += item_data
